...
- [x] 2\. Post an update on your GBP right now — a photo of recent work, a new client launch, anything. Do this weekly going forward. An inactive GBP hurts local pack ranking.  
- [x] 3\. Confirm your GBP primary category is set appropriately. Current: “Website designer.” Consider changing to “Web design company” — the business vs. individual distinction matters for how Google categorizes you.
...
- [x] 8\. Ask every past client to add “Website by Fishtown Web Design” with a link to `https://www.fishtownwebdesign.com` in their site footer. Confirmed already linking: Axel Mechanical, Green Shades Coffee, Shawnee Run Venue. Contact all others.
...
- [x] 9\. Find and fix the **4 four-oh-four errors** — OG image paths updated on 6 pages (`about.html`, `contact.html`, `pp.html`, `pricing.html`, `tos.html`, `faq.html`) → `/public/content/home/logometa.png`. ✅ Done 2026-06-05. Re-crawl in Semrush to clear 4xx.  
- [x] 10\. Fix the **9 broken internal links** across `public/*.html` — partial 2026-05-20: footer links fixed in `about.html`, `charity.html`, `home.html` (`/tos`, `/pp`); `og:image:height` typo fixed in `pricing.html`. **Still open:** Semrush May 2026 crawl still reports 9 broken internals — re-crawl after deploy.  
...
- [x] 12\. Fix **3 duplicate title tags** — partial 2026-05-20: `pp.html` title → `Privacy Policy | Fishtown Web Design`. **Still open:** \~2 duplicates likely on blog/dynamic pages — confirm on re-crawl.  
- [x] 13\. Fix **3 duplicate meta descriptions** — identify which pages share descriptions and differentiate them  
...
- [x] 16\. **Minify CSS and JS** — source files moved to `src/css/` and `src/js/`; `npm run build` minifies to `public/css/` and `public/scripts/` (24–50% CSS reduction); `npm run dev` copies unminified for local development; `compression` middleware enabled in `server.js` for gzip on all responses. ✅ Done 2026-06-05.
...
*Last reviewed: 2026-06-02 (codebase: `public/`, `views/`, `server.js`; Semrush full audit \+ competitor analysis \+ backlink gap run 2026-05-19; technical fixes applied 2026-05-20). Update this block whenever you ship meaningful SEO/IA changes.*
...
| P1 | **Technical audit fixes (from Semrush 2026-05-19):** ~~6 broken internal images~~ ✅, ~~2 sitemap URLs~~ ✅, ~~6 structured data errors~~ ✅, ~~OG 404s (all 6 pages including faq.html)~~ ✅. **Partial:** broken internal links (footer fixes shipped, re-crawl pending), `pp.html` title only (2 duplicate titles remain). **Open:** 9 broken links until re-crawl, 3 duplicate meta descriptions, 61 unminified JS/CSS, 3 low word-count pages, 13 low text-to-HTML pages. | **Partial — in progress** | `public/*.html`, `server.js`, CSS/JS pipeline |
...
2. **`/seo` copy vs E-E-A-T and current ranks** — **Partial (meta done; body copy open)**  
   **Meta/OG fixed (2026-06-02):** `meta description` and `og:description` now say “Philadelphia's local SEO agency” — `#1` claim removed. **Still open:** body copy still needs proof, deliverables, reporting cadence, and scope (no superlatives). Optional: audit visible hero copy for any remaining superlative language.  
